...
const navItems = [
  ...
  { icon: Presentation, label: "Tutor", href: "/pages/tutor" },
...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:static top-0 left-0 z-50 min-h-screen bg-white border-r border-gray-200 flex flex-col
        transform transition-all duration-300 ease-in-out ${collapsed ? "w-20" : "w-64"}
        ...
        <div className={`mt-auto px-4 py-4 border-t border-gray-200 flex items-center ${collapsed ? "justify-center" : "gap-3"}`}>
          {user.image ? (
            <div className="relative w-10 h-10">
              <Image src={user.image} alt="user" fill className="rounded-full border object-cover" />
            </div>
          ) : (
